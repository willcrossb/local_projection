...
def var_boot(A, res, Y, p, homosk, no_const):
    ...
    # Dimensions
    T, n = Y.shape
    T_res = res.shape[0]  # Effective sample size for residuals (need not equal T-p)

    # Draw block of initial T-T_res obs. from real data
    ind_init = np.random.randint(T_res + 1)
    ...
    if homosk:  # I.i.d. bootstrap
        res_boot = res[np.random.randint(T_res, size=(T_res, 1)), :]
    else:  # Wild bootstrap
        res_boot = np.random.randn(T_res, 1) * res

    # Generate VAR(p) data, with residuals and initial conditions as above
    Y_boot = np.concatenate((Y_init, var_sim(A[:, :n * p], c, res_boot, Y_init)))
    return Y_boot

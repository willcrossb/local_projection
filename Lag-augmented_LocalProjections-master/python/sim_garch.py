...
def sim_garch(alpha_0, alpha_1, beta_1, T):
    ...
    epsilons = np.random.randn(T, 1)  # Shocks

    sigma2s = np.zeros(T, 1)
    sigma2s[0] = alpha_0 / (1 - alpha_1 - beta_1)  # Initialize sigma_t^2 at its ergodic mean

    for t in range(1, T):
        sigma2s[t] = alpha_0 + (alpha_1 * epsilons[t-1]**2 + beta_1) * sigma2s[t-1]

    U = np.sqrt(sigma2s) * epsilons
